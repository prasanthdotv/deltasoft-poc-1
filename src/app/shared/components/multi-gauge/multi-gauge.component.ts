...
import { MultiGaugeService } from '@app/shared/services/multi-gauge/multi-gauge.service';
...
export class MultiGaugeComponent implements OnInit {
  ...
  constructor(
    ...
    private barChartService: MultiGaugeService,
  ...
  async initChartDataUpdation(event?) {
    ...
    try {
      ...
      if (!this.isChartOptionsInitialized) {
        ...
        for (let i = 0; i < 4; i++) {
          this.barChartConfig[i] = this.barChartService.updateChartData(this.barChartConfig[i], processedData);
          if (this.echartsInstance[i]) {
            this.echartsInstance[i].setOption(this.barChartConfig[i]);
            this.chartService.storeTestData(this.inputId, this.barChartData);
          }
        }
      }
  ...
  changeBarGraphTheme() {
    for (let i = 0; i < 4; i++) {
      this.barChartConfig[i] = this.barChartService.changeChartTheme(this.barChartConfig[i], this.theme);
      this.echartsInstance[i].setOption(this.barChartConfig[i]);
    }
  }

  onChartInit(ec, id: number) {
    this.echartsInstance[id] = ec;
  }
